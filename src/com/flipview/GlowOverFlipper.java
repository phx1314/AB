...
public class GlowOverFlipper implements OverFlipper {
	...
	public float calculate(float flipDistance, float minFlipDistance,
		...
		if (deltaOverFlip > 0) {
			mBottomEdgeEffect.onPull(deltaOverFlip
					/ (mFlipView.isFlippingVertically() ? mFlipView.getHeight()
							: mFlipView.getWidth()));
	...
	private boolean drawTopEdgeEffect(Canvas canvas) {
		...
		if (!mTopEdgeEffect.isFinished()) {
			...
			if (mFlipView.isFlippingVertically()) {
				mTopEdgeEffect.setSize(mFlipView.getWidth(),
						mFlipView.getHeight());
				canvas.rotate(0);
			} else {
				mTopEdgeEffect.setSize(mFlipView.getHeight(),
						mFlipView.getWidth());
				canvas.rotate(270);
	...
	private boolean drawBottomEdgeEffect(Canvas canvas) {
		...
		if (!mBottomEdgeEffect.isFinished()) {
			canvas.save();
			if (mFlipView.isFlippingVertically()) {
				mBottomEdgeEffect.setSize(mFlipView.getWidth(),
						mFlipView.getHeight());
				canvas.rotate(180);
				canvas.translate(-mFlipView.getWidth(), -mFlipView.getHeight());
			} else {
				mBottomEdgeEffect.setSize(mFlipView.getHeight(),
						mFlipView.getWidth());
				canvas.rotate(90);
