 ...
 * 
 */
public class DragImageView extends MImageView {
	...
	private int current_Top, current_Right, current_Bottom, current_Left, left;// 当前图片上下左右坐标
	...
	/** 构造方法 **/
	public DragImageView(Context context, ViewPager mViewPager) {
		super(context);
	...
	void onTouchMove(MotionEvent event) {
		...
		if (mode == MODE.DRAG) {
			...
			if (isControl_H || isControl_V)
				this.setPosition(this.left + left, top, this.left + right,
						bottom);
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= MAX_W) {
			...
			if (current_Left <= this.left
					&& current_Right >= this.left + screen_W) {
		...
		// 缩小
			...
			// 左边越界
			if (isControl_H && current_Left >= this.left) {
				current_Left = this.left;
				current_Right = this.getRight() - 2 * disX;
				if (current_Right <= this.left + screen_W) {
					current_Right = this.left + screen_W;
					isControl_H = false;// 关闭
				}
			}
			// 右边越界
			if (isControl_H && current_Right <= this.left + screen_W) {
				current_Right = this.left + screen_W;
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			while (current_Width <= screen_W) {
				Log.e("jj", "current_Width=" + current_Width);
				left -= step_H;
				top -= step_V;
				right += step_H;
				bottom += step_V;
